...
const longestString = (arr) => {
    const str = arr.reduce((accum, val) =>{ 
...
const isSimilar = (arr) => {
    ...
    if( arr[0].split('').length !== arr[0].split('').length){
      return false
    }
    let array1 = arr[0].split('').filter((el)=>{
    return arr[1].includes(el);
    })
    if (array1.length === arr[0].split('').length){
      return true
    }
    else {
      return false;
